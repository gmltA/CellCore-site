...
if (form_validate('form')) do {
    ...
    if ($account=db_fetch_array(db_query('SELECT id, username, sha_pass_hash FROM account WHERE username=%s', $v['name'])))
    {
        ...
        if ($pass == $account['sha_pass_hash'])
        {
            ...
            if ($character=db_fetch_array(db_query('SELECT * FROM characters where name=%s',$v['nick'])))
            {
                ...
                db_query('UPDATE characters SET at_login=%s where name=%s',128, $v['nick']);
  ...
  // *********************************
  header('Location: /tool/changerace/?done');
  exit;
...
<h3>Смена расы</h3>
<p>
Хотите поиграть за другую расу?<br>
Нет ничего проще!<br>
Играя у нас, Вы можете в любой момент<br>
исполнить это желание.<br>
Требования:
    <blockquote>
        <table style="border-style: solid; border-color: rgb(177, 195, 210); border-width: 1px; box-shadow: 0px 0px 15px rgb(177, 195, 210); border-collapse: collapse ;">
            <tr><td>&nbsp</td><th>Очки арены</th><th>Эмблемы льда</td></tr>
            <tr><th>x4</th><td>4000</td><td>100</td></tr>
            <tr><th>x1</th><td>3000</td><td>200</td></tr>
            <tr><th>FUN</th><td>4000</td><td>150</td></tr>
        </table>
    </blockquote>
